...
class Building_Extractor(object):
    ...
    def getParameterInfo(self):
     ...
     # Input parameters
      ...
      #setting default value
        ...
        spatial_detail = arcpy.Parameter(displayName="Spatial Detail",name="spatial_detail",datatype="GPLong",parameterType="Required", direction="Input", category = "Segment Mean Shift Parameters")
      #setting default value
        spatial_detail.value=15
        min_segment_size = arcpy.Parameter(displayName="Min Segment Size",name="min_segment_size",datatype="GPLong",parameterType="Required", direction="Input", category = "Segment Mean Shift Parameters")
      #setting default value
        min_segment_size.value=10 
        height = arcpy.Parameter(displayName="Any value less than or equal to this will be converted to 0",name="height",datatype="GPDouble",parameterType="Required", direction="Input", category = "Convert Height Parameters")
      #setting default value
        height.value=2.0
        ...
        binningmethod.filter.list = ["BINNING AVERAGE NONE","BINNING AVERAGE SIMPLE","BINNING AVERAGE LINEAR","BINNING AVERAGE NATURAL_NEIGHBOR","BINNING MINIMUM NONE","BINNING MINIMUM SIMPLE","BINNING MINIMUM LINEAR","BINNING MINIMUM NATURAL_NEIGHBOR","BINNING MAXIMUM NONE","BINNING MAXIMUM SIMPLE","BINNING MAXIMUM LINEAR","BINNING MAXIMUM NATURAL_NEIGHBOR","BINNING IDW NONE","BINNING IDW SIMPLE","BINNING IDW LINEAR","BINNING IDW NATURAL_NEIGHBOR","BINNING NEAREST NONE","BINNING NEAREST SIMPLE","BINNING NEAREST LINEAR","BINNING NEAREST NATURAL_NEIGHBOR"]
	...
	samplingvalue = arcpy.Parameter(displayName="samplingvalue",name="samplingvalue",datatype="GPDouble",parameterType="Required", direction="Input", category = "LAS Dataset To Raster Parameters")
	samplingvalue.value=1



        parameters = [lasdir, demdir, outputdir, spectral_detail, spatial_detail, min_segment_size, height, lidarvalue, binningmethod, rasterouttype, samplingtype, samplingvalue]
        return parameters
    ...
    def execute(self, parameters, messages):
        ...
        fulloutfolder=os.path.join(outfolder, timestamp.strftime('%Y%m%d%H%M%S'))
	...
	for filename in files: #os.listdir(lasdir):
            ...
            arcpy.CreateLasDataset_management(inputfile, outputfile, create_las_prj="NO_FILES")
            lasLyr = arcpy.CreateUniqueName(basename)
            arcpy.management.MakeLasDatasetLayer(outputfile + "d", lasLyr, class_code=1, return_values='LAST RETURN')
            outimg=os.path.join(fulloutfolder,"lr"+basename+".img")
            arcpy.conversion.LasDatasetToRaster(lasLyr, outimg, lidarval,binningmethod, data_type, sampling_type , sampling_value, 1)
            arcpy.CheckOutExtension('Spatial')
            outMinus = Raster(outimg) - Raster(os.path.join(demdir,basename+".img"))
	    ...
	    outSetNull = SetNull(outCon, outCon, "VALUE <= 0")
            outSetNull.save(os.path.join(fulloutfolder,"lrdiff"+basename+".img"))
            arcpy.AddMessage("Segment Mean Shift phase. This will take some time. Be patient.")
            seg_raster = SegmentMeanShift(outSetNull, spectral_detail, spatial_detail,  min_segment_size)
            seg_raster.save(os.path.join(fulloutfolder,"lrdiffgt2is"+basename+".img"))
